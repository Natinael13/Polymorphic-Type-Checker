...
fun literal (l) =  
          (case l
            ...
            | (PAIR (v, NIL))  => listtype (literal v)
            | (PAIR (v, v'))   => 
              let val vlstty = listtype (literal v)
              in
                if eqType(vlstty, literal v')  then 
                  vlstty
                else 
              raise TypeError "pair not empty error"
              end
            | (CLOSURE (x, y)) => raise LeftAsExercise "No need to implement"
            | (PRIMITIVE x)    => raise LeftAsExercise "No need to implement"
            | (ARRAY x)        => raise LeftAsExercise "No need to implement")

(* type checking for {\tuscheme} ((prototype)) 375 *)
fun typeof (e, delta, gamma) =
  let fun ty (LITERAL v)                = literal v
        | ty (VAR x)                    = find (x, gamma)
        | ty (SET (x, e1))              = 
           let val tau_e1 = ty e1 
               val tau_x = find (x, gamma)
            in if eqType (tau_e1, tau_x) then
                 tau_e1
               else
        ...
        | ty (IFX (e1, e2, e3))          = 
            let val tau_e1 = ty e1
                val tau_e2 = ty e2
                val tau_e3 = ty e3
            in  if eqType (tau_e1, booltype) then
                  if eqType (tau_e2, tau_e3) then
                    tau_e3
                  else
                    raise TypeError 
                    ("IFX error " ^ typeString tau_e1 ^ 
                    typeString tau_e2 ^ typeString tau_e3)
                else
                  raise TypeError "IFX error" 
            end
        | ty (WHILEX (e1, e2))           = 
           let val tau_e1 = ty e1 
               val tau_e2 = ty e1 
            in if eqType (tau_e1, booltype) then
                 ...
                 raise TypeError "While error"
            end
        | ty (BEGIN es)                  = 
           let val ty_es = map ty es
               val ty_last = List.last ty_es
           in ty_last
           end
        | ty (APPLY (f, actuals))        = 
              ...
              (case ty f
                of (FUNTY (formaltypes, resulttype)) => 
                  if eqTypes (actualtypes, formaltypes) then
                    ...
                    raise TypeError "APPLY error1"
                | _                                  => 
                    raise TypeError 
                    ("APPLY error2  " ^ typeString (ty f) ^ "\n"))
          end
        | ty (LETX (LET, ls, ebody))      = 
           let val exps     = map snd ls
               val names    = map fst ls
               val tau_list = map ty exps
               val gamma2   = bindList (names, tau_list, gamma)
            in typeof (ebody, delta, gamma2)
            end  
        | ty (LETX (LETSTAR, [], e1))      = ty e1 
        | ty (LETX (LETSTAR, x :: xs, e1)) = 
           ty (LETX (LET, [x], LETX (LETSTAR, xs, e1)))
        | ty (LETRECX (ls, ebody))         = 
           let val exps          = map snd ls
               val names_tyex    = map fst ls
               val names         = map fst names_tyex
               val tyexs         = map snd names_tyex
               val tau_list      = map (fn (x) => asType (x, delta)) tyexs
               val gamma2        = bindList (names, tau_list, gamma)
               fun zip ([], [])       = []
                 | zip (x::xs, y::ys) = (x,y) :: zip (xs,ys)
                 | zip _              = raise TypeError "Zip error"
               val letFormls     = zip (names, exps)    
               val bindedLetForm = LETX (LET, letFormls, ebody)
            in typeof (bindedLetForm, delta, gamma2)
            end
        | ty (LAMBDA le)                  = 
           let val lst       = fst le
               val exps      = map snd lst
               val names     = map fst lst
               val tau_list  = map (fn (x) => asType (x, delta)) exps
               val gamma2    = bindList (names, tau_list, gamma)
               val finaltype = typeof (snd le, delta, gamma2)
            in FUNTY (exps, finaltype)  
            end   
        | ty (TYLAMBDA (ls, e1))          = 
           let val freevars     = freetyvarsGamma gamma
               val lsInFreevars = 
               List.exists (fn x => List.exists (fn y => x = y) freevars) ls
               val tau_list     = map (fn x => TYPE ) ls
               val delta2       = bindList (ls, tau_list, delta)
               val e1_tau       = typeof (e1, delta2, gamma)   
           in 
             if lsInFreevars then
               raise TypeError "TYlambda error"
             else
               FORALL (ls, e1_tau)
           end
        | ty (TYAPPLY (e1, ls))           = 
           let val lskindchecked = map (fn (x) => asType (x, delta)) ls
           in
              (case ty e1
                of (FORALL(namels, t)) => 
                  instantiate (ty e1, ls, delta)
                | _                    => 
                    raise TypeError
                    ("TYAPPLY error2  " ^ typeString (ty e1) ^ "\n"))
           end
  in  ty e
  end
  ...
  case d
    of
      VAL (x, e)            => 
       ...
       | EXP e              =>
          typdef (VAL ("it", e), delta, gamma)
       | DEFINE (f, te, le) => 
          let val lst       = fst le
              val exps      = map snd lst
          in typdef (VALREC (f, FUNTY (exps, te), LAMBDA le), delta, gamma)
          end
       | VALREC (x, te, e)  => 
          let val kchecking = asType (te, delta)
              val gamma2    = bind (x, te, gamma)
              val tau_e     = typeof (e, delta, gamma2)
          in if eqType (te, tau_e) then
                (gamma2, typeString te)
              else
