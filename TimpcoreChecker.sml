...
(* function [[ty]], checks type of expression, given $\itenvs$ ((prototype)) 357 *)
      | ty (AAT (a, i))        = 
          (case (ty a, ty i)
            of (ARRAYTY INTTY, INTTY)      => INTTY
            | (ARRAYTY BOOLTY, INTTY)      => BOOLTY
            | (ARRAYTY UNITTY, INTTY)      => UNITTY
            | (ARRAYTY (ARRAYTY x), INTTY) => ARRAYTY x
            | _                            => raise TypeError "ArrayAt error")
      | ty (APUT (a, i, e))    = 
          (case (ty a, ty i, ty e)
            of (ARRAYTY INTTY, INTTY, INTTY)          => INTTY
            | (ARRAYTY BOOLTY, INTTY, BOOLTY)         => BOOLTY
            | (ARRAYTY UNITTY, INTTY, UNITTY)         => UNITTY
            | (ARRAYTY (ARRAYTY x), INTTY, ARRAYTY y) => 
                             if eqType (x, y) then
                               ARRAYTY y
                             else
                               raise TypeError "ArrayPut case 4 error"
            | _                                       => 
                               raise TypeError "ArrayPut error")
      | ty (AMAKE (len, init)) = 
          (case (ty len, ty init)
            of (INTTY, INTTY)    => ARRAYTY INTTY
            | (INTTY, BOOLTY)    => ARRAYTY BOOLTY
            | (INTTY, UNITTY)    => ARRAYTY UNITTY
            | (INTTY, ARRAYTY x) => ARRAYTY (ARRAYTY x)
            | _                  => raise TypeError "ArrayAt error")
      | ty (ASIZE a)           = 
          (case ty a
            of (ARRAYTY INTTY)      => INTTY
            | (ARRAYTY BOOLTY)      => INTTY
            | (ARRAYTY UNITTY)      => INTTY
            | (ARRAYTY (ARRAYTY x)) => INTTY
            | _                     => raise TypeError "ArraySize error")     
